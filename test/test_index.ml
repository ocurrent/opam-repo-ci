...
let test_simple () =
  ...
  Index.record_job ~repo ~hash ~variant:"analysis" ~job_id:(Some "job1");
  Index.record_job ~repo ~hash ~variant:"alpine" ~job_id:None;
  Index.set_status ~repo ~hash `Pending;
  ...
  Index.record_job ~repo ~hash ~variant:"alpine" ~job_id:(Some "job2");
  Index.set_status ~repo ~hash `Failed;
