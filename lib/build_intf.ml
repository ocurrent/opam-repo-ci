...
module type S = sig
  ...
  (** [list_revdeps ~variant ~opam_version ~pkgopt ~new_pkgs ~base ~master ~after commit]
      ...
      [new_pkgs] is used to filter out already-tested packages that are
      being added by the PR as well.
