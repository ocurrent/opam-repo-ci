...
(** [msg_of_errors] returns a string describing all the linting [errors].
    ...
    The output is human-friendly, grouping errors by package and adding
    indentation and newlines for readability. Use {!msg_of_errors_machine} for
    output that is machine-friendly. *)
let msg_of_errors (errors : (OpamPackage.t * error) list) =
...
(** [msg_of_errors_machine] returns a string describing all the linting [errors].
    ...
    The output is a simple list of error messages, one per line. For a more
    human-friendly output, use {!msg_of_errors}. *)
let msg_of_errors_machine (errors : (OpamPackage.t * error) list) =
